...
SELECT
    ...
    f1.target_user_id,
    f2.initiator_user_id
FROM users u
JOIN friend_requests f1 ON u.id=f1.target_user_id 
JOIN friend_requests f2 ON u.id=f2.initiator_user_id 
WHERE u.id = 1;
...
SELECT
	DISTINCT m1.body
FROM users u
JOIN messages m1 ON u.id = m1.from_user_id OR u.id = m1.to_user_id
WHERE u.id = 1;
...
SELECT
	m.filename AS media_type,
    m.body AS media_text,
    COUNT(l.media_id) AS `count_like`
FROM media m
JOIN users u ON m.user_id = u.id
JOIN likes l ON m.id = l.media_id
WHERE u.id = 2 
GROUP BY m.filename, m.body;

-- Получите количество групп у пользователей
SELECT
	u.id,
	CONCAT(u.firstname, " ", u.lastname) AS fullname,
	COUNT(c.name) AS `count_groupp`
FROM communities c
JOIN users_communities uc ON c.id = uc.user_id
...
SELECT
    ...
    CONCAT(u.firstname, " ", u.lastname) AS fullname,
    COUNT(m.body),
    DENSE_RANK() OVER raiting AS `dense_rank`
FROM users u    
JOIN messages m ON u.id = m.from_user_id OR u.id = m.to_user_id
GROUP BY u.id
WINDOW raiting AS (ORDER BY COUNT(m.body))
ORDER BY `dense_rank`;
...
-- первый вариант (подсмотренный)
SELECT
 body,
 created_at,
 LAG(created_at,1,0) OVER(PARTITION BY TIMESTAMPDIFF(SECOND, created_at,created_at)) AS `lag`,
 LEAD(created_at) OVER(PARTITION BY TIMESTAMPDIFF(SECOND, created_at,created_at)) AS `lead`
FROM messages
...
SELECT
 body,
 created_at,
 LAG(created_at) OVER() AS `lag`,
 LEAD(created_at) OVER() AS `lead`
FROM messages
